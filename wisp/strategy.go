...
func (s *AITraderStrategy) tick(ctx context.Context, n *nobitexClient, ai *openAIClient, risk *riskGate, src, dst, market string, paper bool, minConfidence float64) error {
	...
	if s.gate == nil { return fmt.Errorf("approval gate is not configured") }
	proposal, err := s.gate.create(market, decision)
	if err != nil { return fmt.Errorf("create approval proposal: %w", err) }
	...
	if !paper { return fmt.Errorf("execution mode is not paper; refusing automatic order") }
	return nil
}
